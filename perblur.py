...
train_data, valid_data, test_data, inclination_data, user_data, dataset_name= read_dataset_to_obfuscate(f"{ROOT_DIR_STR}/obfuscation/{dataset_name}")
dataset_src = pd.concat([train_data, valid_data], ignore_index=True)
interaction_matrix,user_info,token2item,token2user = transform_dataframe_to_sparse(dataset_src)
...
#%% 
import pickle
pickle.dump(userlist,open(f"{out_dataset_dir}/{obf_dataset_name}/ml-{kth}-personalized-counts.pkl","wb"))
#%%
...
pickle.dump(indicative,open(f"{out_dataset_dir}/{obf_dataset_name}/ml-indicative.pkl","wb"))
# %%
X=interaction_matrix.copy()#sp.load_npz(f"{ROOT_DIR_STR}/obfuscation/ml-1m/interactions.npz")
...
for iduser,user_row in enumerate(obf_matrix):
    data_obf.append([user2token[iduser],list(item2token.loc[user_row.nonzero()[1]].values)]) 
...
save_recbole_data(train_data_obf,valid_data_obf,test_data,f"{out_dataset_dir}/{obf_dataset_name}")
